...
static void draw_edge(uint32_t *gram, obj_info_t *obj_info, uint32_t index, uint16_t color)
{
	...
	if(!(abs(chang1-x1)<=15&&abs(chang2-x2)<=15&&abs(kuan1-y1)<=30&&abs(kuan2-y2)<=30))
		{
			P_NUM++;
		}
	num_String(P_NUM, &C_NUM);
	chang1=x1;chang2=x2;kuan1=y1;kuan2=y2;
	lcd_draw_string(x1 + 1, y1 + 1, &C_NUM, RED);
...
void Num_str(uint64_t x, char *s) // int x:数字数组；char *s：数字字符串数组
{
    int i, j, n = 0;
	uint64_t v;
    char c;
...
int main(void)
{
    ...
    while (1)
    {
		...
		lcd_draw_string(135, 2, &C_NUM, GREEN);
		lcd_draw_string(180, 2, "SPF:", BLUE);
		...
		printf("   origin %ld\n",D_time);
		Num_str(D_time, &D_NUM);
		printf("   hi %s\n",D_NUM);
		Change_string(&D_NUM);
		printf("   hello  %s\n",D_NUM);
		lcd_draw_string(214, 2, &D_NUM, RED);
    }
}
